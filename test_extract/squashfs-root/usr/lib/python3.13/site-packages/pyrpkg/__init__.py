...
class Commands(object):
    ...
    def __init__(self, path, lookaside, lookasidehash, lookaside_cgi,
                 ...
                 lookaside_attempts=None, lookaside_delay=None, koji_offline=False):
        ...
        # prevent connecting to Koji
        self.koji_offline = koji_offline
    ...
    def default_branch_merge(self):
        ...
        raise rpkgError('Unable to find Git repo. Use --release\n')
    ...
    def load_repo_name(self):
        ...
        try:
            ...
            self.log.debug('Failed to get repository name from Git url or pushurl')
    ...
    def load_nvr(self):
        ...
        name = self.spec[:-5]  # remove suffix '.spec'
        self._nvr = '%s-%s-%s' % (name, self.ver, self.rel)
    ...
    def uses_autorelease(self):
        if self._uses_autorelease is None:
            self.load_rpmautospec()
        return self._uses_autorelease

    @property
    def uses_autochangelog(self):
        if self._uses_autochangelog is None:
            self.load_rpmautospec()
        return self._uses_autochangelog
    ...
    def load_rpmautospec(self):
        ...
        if specfile_uses_rpmautospec:
            ...
            self._uses_autochangelog = specfile_uses_rpmautospec(
                specfile_path, check_autorelease=False, check_autochangelog=True
            )
    ...
    def mock_results_dir(self):
        if self.results_dir == 'subdir':
            results_dir = "results/mock_%s" % self.repo_name
        else:
            results_dir = "results_%s" % self.repo_name
        return os.path.join(self.path, results_dir, self.ver, self.rel)
    ...
    def sources(self, outdir=None, force=False):
        ...
        # Backward compatibility for scripts which require "source"
        # command to download all files
        if os.environ.get("RPKG_SOURCES_FORCE", "0") != "0":
            force = True
    ...
    def build(self, skip_tag=False, scratch=False, background=False,
        ...
        if nvr_check and not scratch and not url.endswith('.src.rpm') and not self.uses_autorelease:
    ...
    def lint(self, info=False, rpmlintconf=None):
        ...
        rpm_globs = set()
        srpm_base = "%s.src.rpm" % self.nvr

        mockdir = self.mock_results_dir
        ...
        for rpm_glob in rpm_globs:
            for path in glob.glob(rpm_glob):
                if '-debuginfo' not in path and '-debugsource' not in path:
                    rpms.add(path)
        if not rpms:
        ...
        info_switch = '-i'  # switch that uses rpmlint version 1.x
    ...
    def mock_config(self, target=None, arch=None, standalone=None):
        ...
        :param str arch: an alternative arch, otherwise local system arch will
            be used.
        :param bool standalone: If True, mock_config is run in standalone mode,
            not as part of the mockbuild command
        ...
        if not build_target:
            if standalone:
                raise rpkgError('Unknown build target: %s\n'
                                'Consider using/change the --target option' % target)
            else:
                # 'target' passed to the 'mock_config' method is always None when
                # not standalone - use self.target
                raise rpkgError('Unknown build target: %s\n'
                                'Consider using/change the "x-pkg --release" option' % self.target)
    ...
    def upload(self, files, replace=False, offline=False):
        ...
        if len(files) == 0:
            raise NoSourcesError('There are no sources')
    ...
    def prep(self, arch=None, builddir=None, buildrootdir=None, define=None,
             extra_args=None, check_deps=None):
        """Run ``rpmbuild -bp``
        ...
        :param list extra_args: additional arguments that are passed to
            the rpmbuild command.
        :param bool check_deps: check dependencies. No, by default.
        ...
        if not check_deps:
            cmd.append('--nodeps')
        cmd.extend(['-bp', os.path.join(self.layout.specdir, self.spec)])
        # Run the command
        try:
            self._run_command(cmd)
        except rpkgError:
            if not check_deps:
                self.log.info("Hint: the previous command didn't check dependencies. "
                              "This can be enabled by adding argument '--check-deps'")
            raise
    ...
    def remote(self, remote_name, repo_name, anon=False):
        ...
        :param bool anon: anonymous remote will be added instead of named remote
        """

        if repo_name and self.distgit_namespaced and '/' not in repo_name:
            if self.ns:
                repo_name = '%s/%s' % (self.ns, repo_name)
            else:
                repo_name = 'rpms/%s' % repo_name
        ...
        giturl = self._get_namespace_giturl(repo_name or self.ns_repo_name)
        remote_url = anongiturl if anon else giturl

        ret, _, _ = self._run_command(['git', 'remote', 'add', remote_name, remote_url])
        if ret == 0:
            self.log.info("Remote '{}' for '{}' was added.".format(remote_name, remote_url))
    ...
    def pre_push_check(self, ref):
        ...
        try:
            ...
            try:
                ...
                return
            ret, stdout, stderr = self._run_command(cmd, cwd=clone_dir, return_text=True,
                                                    return_stdout=True, return_stderr=True)
            if ret != 0:
