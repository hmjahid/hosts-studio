...
class _Mock(HelpersMixin):
    ...
    def build(self, filename):
        ...
        if not Settings.verbose and " -q" not in cmd:
            cmd += ' | grep -E "Results and/or logs|ERROR" '
        ...
        if rc == "0":
            ...
            error = ReviewError("mock build failed, see logs in %s" % self.resultdir)
    ...
    def init(self, force=False):
        ...
        for option in shlex.split(self.get_mock_options()):
            allowed_options = ("--uniqueext", "--configdir", "--disable-plugin")
            if option.startswith(allowed_options):
                cmd.append(option)
        cmd.append("--init")
        self._run_cmd(cmd, "Init")
    ...
    def rpmlint_rpms(self, rpms):
        ...
        def filter_output(output):
            ...
            lines = output.split("\n")
            line = ""
            while not line.startswith("rpmlint:") and lines:
                line = lines.pop(0)
            text = ""
            for line in lines:
                if line.startswith("<mock-"):
                    line = line[line.find("#") :]
                if line.startswith("rpmlint-done:"):
                    break
                text += line + "\n"
            return text
