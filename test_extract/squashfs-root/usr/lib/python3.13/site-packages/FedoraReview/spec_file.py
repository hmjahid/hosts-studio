...
class SpecFile(object):
    ...
    def _process_fonts_pkg(self):
        ...
        for line in self.lines:
            if "%_font_pkg" not in line:
                expanded.append(line)
            else:
                expanded.extend(rpm.expandMacro(line).split("\n"))
        self.lines = expanded
    ...
    def _parse_files(self, pkg_name):
        ...
        if not pkg_name:
            pkg_name = self.name
        lines = None
        for line in [line.strip() for line in self.lines]:
            if lines is None:
    ...
    def get_files(self, pkg_name=None):
        ...
        if files is None:
            return None
        return [line for line in [f.strip() for f in files.split("\n")] if line]
