...
def map_obj_to_commands(want, have, module):
    ...
    if module.params["state"] == "absent":
        ...
        if have:
            if want["other_config"] != have["other_config"]:
                for k, v in iteritems(want["other_config"]):
                    if (
                        ...
                        if v is None:
                            templatized_command = (
                                "%(ovs-vsctl)s -t %(timeout)s remove port %(port)s other_config "
                                + k
                            )
                            command = templatized_command % module.params
                            commands.append(command)
            ...
            if want["external_ids"] != have["external_ids"]:
                for k, v in iteritems(want["external_ids"]):
                    if (
                        ...
                        if v is None:
                            templatized_command = (
                                "%(ovs-vsctl)s -t %(timeout)s remove port %(port)s external_ids "
                                + k
                            )
                            command = templatized_command % module.params
                            commands.append(command)
            ...
            templatized_command = "%(ovs-vsctl)s -t %(timeout)s add-bond %(bridge)s %(port)s"
            command = templatized_command % module.params
...
def map_config_to_obj(module):
    ...
    if module.params["port"] in out.splitlines():
        ...
        templatized_command = "%(ovs-vsctl)s -t %(timeout)s get Port %(port)s other_config"
        command = templatized_command % module.params
        rc, out, err = module.run_command(command, check_rc=True)
