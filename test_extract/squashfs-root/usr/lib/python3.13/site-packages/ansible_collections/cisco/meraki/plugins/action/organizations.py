...
class Organizations(object):
    ...
    def get_object_by_id(self, id):
        ...
        try:
            ...
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = get_dict_result(items, 'organizationId', id)
        except Exception as e:
            print("Error: ", e)
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "organization_id") or self.new_object.get("organizationId")
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("organizationId")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters(
                    "The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(organizationId=_id))
            if _id:
    ...
    def update(self):
        ...
        id = id or self.new_object.get("organizationId")
        name = self.new_object.get("name")
        result = None
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("organizationId")
            if id_:
                self.new_object.update(dict(organizationId=id_))
        result = self.meraki.exec_meraki(
            family="organizations",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = Organizations(self._task.args, meraki)
