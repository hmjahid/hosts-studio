...
required_if = [
    ("state", "present", ["networkId", "staticDelegatedPrefixId"], True),
    ("state", "absent", ["networkId", "staticDelegatedPrefixId"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class NetworksAppliancePrefixesDelegatedStatics(object):
    ...
    def get_params_by_id(self, name=None, id=None):
        ...
        if self.new_object.get('networkId') is not None or self.new_object.get('network_id') is not None:
            new_object_params['networkId'] = self.new_object.get('networkId') or \
                self.new_object.get('network_id')
        if self.new_object.get('staticDelegatedPrefixId') is not None or self.new_object.get('static_delegated_prefix_id') is not None:
            new_object_params['staticDelegatedPrefixId'] = self.new_object.get('staticDelegatedPrefixId') or \
                self.new_object.get('static_delegated_prefix_id')
        return new_object_params
    ...
    def get_object_by_id(self, id):
        ...
        try:
            ...
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = get_dict_result(items, 'staticDelegatedPrefixId', id)
        except Exception as e:
            print("Error: ", e)
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "static_delegated_prefix_id") or self.new_object.get("staticDelegatedPrefixId")
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("staticDelegatedPrefixId")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters(
                    "The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(staticDelegatedPrefixId=_id))
            if _id:
    ...
    def update(self):
        ...
        id = id or self.new_object.get("staticDelegatedPrefixId")
        name = self.new_object.get("name")
        result = None
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("staticDelegatedPrefixId")
            if id_:
                self.new_object.update(dict(staticDelegatedPrefixId=id_))
        result = self.meraki.exec_meraki(
            family="appliance",
            function="updateNetworkAppliancePrefixesDelegatedStatic",
    ...
    def delete(self):
        ...
        result = self.meraki.exec_meraki(
            family="appliance",
            function="deleteNetworkAppliancePrefixesDelegatedStatic",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = NetworksAppliancePrefixesDelegatedStatics(self._task.args, meraki)
