...
required_if = [
    ("state", "present", ["idpId", "organizationId"], True),
    ("state", "absent", ["idpId", "organizationId"], True),
]
required_one_of = []
mutually_exclusive = []
required_together = []
...
class OrganizationsSamlIdps(object):
    ...
    def get_params_by_id(self, name=None, id=None):
        ...
        if self.new_object.get('organizationId') is not None or self.new_object.get('organization_id') is not None:
            new_object_params['organizationId'] = self.new_object.get('organizationId') or \
                self.new_object.get('organization_id')
        if self.new_object.get('idpId') is not None or self.new_object.get('idp_id') is not None:
            new_object_params['idpId'] = self.new_object.get('idpId') or \
                self.new_object.get('idp_id')
        return new_object_params
    ...
    def get_object_by_id(self, id):
        ...
        try:
            ...
            if isinstance(items, dict):
                if 'response' in items:
                    items = items.get('response')
            result = get_dict_result(items, 'idpId', id)
        except Exception as e:
            print("Error: ", e)
            result = None
        return result
    ...
    def exists(self):
        ...
        o_id = o_id or self.new_object.get(
            "idp_id") or self.new_object.get("idpId")
        ...
        if name_exists:
            _id = prev_obj.get("id")
            _id = _id or prev_obj.get("idpId")
            if id_exists and name_exists and o_id != _id:
                raise InconsistentParameters(
                    "The 'id' and 'name' params don't refer to the same object")
            if _id:
                self.new_object.update(dict(id=_id))
                self.new_object.update(dict(idpId=_id))
            if _id:
    ...
    def update(self):
        ...
        id = id or self.new_object.get("idpId")
        name = self.new_object.get("name")
        result = None
        if not id:
            ...
            if prev_obj_name:
                ...
                id_ = id_ or prev_obj_name.get("idpId")
            if id_:
                self.new_object.update(dict(idpId=id_))
        result = self.meraki.exec_meraki(
            family="organizations",
            function="updateOrganizationSamlIdp",
    ...
    def delete(self):
        ...
        result = self.meraki.exec_meraki(
            family="organizations",
            function="deleteOrganizationSamlIdp",
...
class ActionModule(ActionBase):
    ...
    def run(self, tmp=None, task_vars=None):
        ...
        obj = OrganizationsSamlIdps(self._task.args, meraki)
