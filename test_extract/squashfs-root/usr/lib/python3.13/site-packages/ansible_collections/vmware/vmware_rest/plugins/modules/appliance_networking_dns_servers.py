...
def prepare_argument_spec():
    ...
    argument_spec["mode"] = {"type": "str", "choices": ["dhcp", "is_static"]}
    argument_spec["server"] = {"type": "str"}
    argument_spec["servers"] = {"type": "list", "elements": "str"}
    argument_spec["state"] = {
        "type": "str",
        "choices": ["add", "set", "test"],
        "default": "set",
    }
...
def build_url(params):
    return (
        "https://{vcenter_hostname}" "/api/appliance/networking/dns/servers"
    ).format(**params)
...
async def _add(params, session):
    ...
    subdevice_type = get_subdevice_type("/api/appliance/networking/dns/servers")
...
async def _test(params, session):
    ...
    subdevice_type = get_subdevice_type(
        "/api/appliance/networking/dns/servers?action=test"
    )
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
