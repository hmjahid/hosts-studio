...
options:
    domain:
        description:
        - Domain to add. Required with I(state=['add'])
        type: str
    domains:
        description:
        - List of domains. Required with I(state=['set'])
        elements: str
        type: list
    session_timeout:
        description:
...
RETURN = r"""
# content generated by the update_return_section callback# task: Add another domain configuration
value:
  description: Add another domain configuration
  returned: On success
  sample: {}
  type: dict
"""
...
def prepare_argument_spec():
    ...
    argument_spec["domain"] = {"type": "str"}
    argument_spec["domains"] = {"type": "list", "elements": "str"}
    argument_spec["state"] = {
        "type": "str",
        "choices": ["add", "set"],
        "default": "set",
...
async def _add(params, session):
    _in_query_parameters = PAYLOAD_FORMAT["add"]["query"].keys()
    payload = prepare_payload(params, PAYLOAD_FORMAT["add"])
    subdevice_type = get_subdevice_type("/api/appliance/networking/dns/domains")
